/* =============================================================================
  ...
  Product: PoziTone module for AudioAddict network
  Author: PoziWorld
...
const
  ...
  , strFavoriteButtonQuerySelector = '#wp-track-vote-buttons .vote-btn.up'
  ...
  , strStationName = document.getElementById( 'channel-title' ).innerText
  , $stationDescription = document.querySelector( '.current-channel .description' )
  , strStationNamePlusDesc = document.contains( $stationDescription )
      ? strStationName + ': ' + $stationDescription.innerText
      : strStationName

  , $stationLogo = document.getElementById( 'art' ).getElementsByTagName( 'img' )[0]
  , $playStopButton = document.getElementById( 'ctl-play' )
  , $playerStatusIndicator = $playStopButton.children[ 0 ]
  , $muteUnmuteButton = document.getElementById( 'btn-volume' )
  , $trackInfo = document.querySelectorAll( '.title-container .title' )[0]
  , $kbpsInfo = document.querySelector( '#settings-bitrate input:checked' )
  , $volumeHandle = document.getElementsByClassName( 'handle' )[0]
  ...
  , intKbps = document.contains( $kbpsInfo )
...
var
  ...
  getPlayerStatus : function( boolReturnStatus ) {
    if ( document.contains( $playerStatusIndicator ) ) {
      ...
      var strPlayStopButtonClassAttr = $playerStatusIndicator.className
        ...
        , strWantedClass = ( intWantedClassPosition !== -1 )
            ...
            ? strPlayStopButtonClassAttr.substr(
  ...
  getPlayerVolume : function() {
    if ( document.contains( $volumeHandle ) ) {
      PageWatcher.objPlayerInfo.intVolume = parseInt( $volumeHandle.style.left.replace( '%', '' ) );
    }
  }
  ,
  ...
  processButtonClick_mute : function() {
    PageWatcher.objPlayerInfo.intVolumeBeforeMuted = ( document.contains( $volumeHandle ) )
  ...
  changeVolume : function( strDirection ) {
    ...
    if (  strDirection === 'up' && intVolume >= 1
  ...
  initPlayerStatusObserver : function() {
    var $target = $playerStatusIndicator
      , objOptions = {
            attributes : true
  ...
  initFavoriteButtonContainerObserver : function() {
    $favoriteButtonContainer = document.getElementById( strFavoriteButtonContainerId );
      ...
      , funcCallback = function( arrMutations ) {
          for ( var i = 0, l = arrMutations.length; i < l; i++ ) {
            ...
            if ( arrAddedNodes.length ) {
              ...
              var $tempFavoriteButton = arrAddedNodes[ 0 ].children[ 1 ].children[ 0 ];

              if ( $tempFavoriteButton.classList.contains( strFavoriteButtonClass ) ) {
                $favoriteButton = document.querySelector( strFavoriteButtonQuerySelector );
...
PageWatcher.getVolumeDeltaSettings = GeneralPageWatcher.getVolumeDeltaSettings;
PageWatcher.processButtonClick_volumeUp = GeneralPageWatcher.processButtonClick_volumeUp;
PageWatcher.processButtonClick_volumeDown = GeneralPageWatcher.processButtonClick_volumeDown;
...
$trackInfo.addEventListener(
  ...
  , function( objEvent ) {
      ...
      if (  strNewTrackInfo !== strTrackInfoVolumeMuted
